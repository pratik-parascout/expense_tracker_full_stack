...
document.addEventListener('DOMContentLoaded', function () {
  ...
  axios
    .get('http://13.203.1.119:3000/expense/isPremium', {
...
function addExpense(amount, description, category) {
  ...
  axios
    .post('http://13.203.1.119:3000/expense/add-expense', expenseData, {
      headers: { Authorization: token },
    })
    .then((response) => {
      // fetchExpenses();
...
function deleteExpense(id, listItem) {
  const token = localStorage.getItem('token');
  axios
    .delete(`http://13.203.1.119:3000/expense/expenses/${id}`, {
      headers: { Authorization: token },
    })
    .then((response) => {
      listItem.remove();
...
function buyPremium() {
  ...
  axios
    .post(
      'http://13.203.1.119:3000/expense/create-premium-order',
      {},
      {
        headers: { Authorization: token },
      }
    ...
    .then((response) => {
      ...
      const options = {
        ...
        handler: function (response) {
          axios
            .post(
              'http://13.203.1.119:3000/expense/payment-success',
              {
...
function showLeaderboard() {
  const token = localStorage.getItem('token');
  axios
    .get('http://13.203.1.119:3000/expense/leaderboard', {
      headers: { Authorization: token },
    })
    .then((response) => {
...
function fetchExpenses(page = 1) {
  ...
  axios
    .get(
      `http://13.203.1.119:3000/expense/expenses?page=${page}&limit=${itemsPerPage}`,
      {
        headers: { Authorization: token },
      }
    )
